#!/usr/bin/env bash
# ==============================================================================
# Installer for Focus Hub (daemon0.focus-hub)
...
DRY_RUN=0
TARGET_PLUGIN_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/omarchy/plugins/daemon0.focus-hub"
BACKUP_BASE="${XDG_STATE_HOME:-$HOME/.local/state}/omarchy/backups/daemon0.focus-hub"
...
check_deps() {
  echo "Checking requirements for Focus Hub..."
  local missing=0
...
do_install() {
  ...
  echo "Installing Focus Hub to $TARGET_PLUGIN_DIR..."
  ...
  echo "Focus Hub installed successfully!"
  echo "To enable in Omarchy bar: omarchy plugin enable daemon0.focus-hub"
}

do_status() {
  echo "=== Focus Hub Status ==="
  if [[ -d "$TARGET_PLUGIN_DIR" ]]; then
  ...
  if command -v omarchy >/dev/null 2>&1; then
    ...
    omarchy plugin list 2>/dev/null | grep "daemon0.focus-hub" || echo "Not registered/disabled"
  fi
}

do_uninstall() {
  echo "Uninstalling Focus Hub..."
  if command -v omarchy >/dev/null 2>&1; then
    run_cmd omarchy plugin disable "daemon0.focus-hub" >/dev/null 2>&1 || true
  fi
  ...
  echo "Focus Hub uninstalled cleanly. Reversible backups preserved under $BACKUP_BASE."
}
