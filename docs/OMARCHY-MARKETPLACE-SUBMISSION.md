...
This document outlines the procedure to propose `omarchy-focus-hub` (`daemon0.focus-hub`) for inclusion in the official Omarchy Plugin Marketplace.

## Prerequisites

1. **Public Repository**: Ensure this repository is published publicly on GitHub under your account:
   `https://github.com/YOUR-USERNAME/omarchy-focus-hub.git`
...
3. **End-to-End Installation Testing**:
   ...
   omarchy plugin add https://github.com/YOUR-USERNAME/omarchy-focus-hub.git --enable --yes

   # 2. Verify status and discovery
   omarchy plugin list | grep "daemon0.focus-hub"

   # 3. Test runtime disable and enable
   omarchy plugin disable daemon0.focus-hub
   omarchy plugin enable daemon0.focus-hub

   # 4. Test plugin update
   omarchy plugin update daemon0.focus-hub --yes
...
2. Prepare your submission details:
   - **Repository URL**: `https://github.com/YOUR-USERNAME/omarchy-focus-hub.git`
   - **Plugin Identifier**: `daemon0.focus-hub`
